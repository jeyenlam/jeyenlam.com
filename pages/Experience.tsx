        ...
        <div className="mt-5 flex flex-col lg:flex-row justify-center items-center">
          ...
          <div className=' text-xs sm:text-sm lg:relative -left-16 md:w-4/6 xl:w-3/6 text-zinc-800'>
            {experiences.map((experience, index) => {return (
              <div key={index} className='p-4  rounded-3xl -b-2 lg:-b-4 -zinc-900 mt-4 bg-indigo-100'>
                <div className='flex flex-col sm:flex-row gap-2 sm:gap-4 items-start sm:items-center'>
                  {/* Title */}
                  <h1 className='md:text-lg font-medium sm:font-semibold py-1 sm:py-2 -zinc-900  bg-lime-200 rounded-full lg:-r-4 px-2 sm:px-4'>{experience.title}</h1>
                  {/* Company */}
                  <h1 className='text-[10px] sm:text-sm px-2 sm:px-4  -zinc-900 lg:-r-4 hover:font-bold bg-indigo-400 rounded-full'><Link target='_blank' href={experience.company.url}>{experience.company.name}</Link></h1>
                </div>
                {/* Duration */}
                <p className='m-1 sm:m-2 text-[9px] sm:text-xs font-medium text-zinc-500'>{experience.duration}</p>
                <ul className='bg-slate-50  -zinc-900 p-2 sm:p-4 rounded-3xl'>
