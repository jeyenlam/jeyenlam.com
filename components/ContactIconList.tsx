      ...
      ( isBox === true ?
        <ul className='pt-4 flex gap-2 items-center justify-center'>
          {contacts.map((contact, index)=>{ return (
            <Link key={index} href={contact.url} className='box flex items-center gap-x-2 rounded-lg hover:translate-y-[2px] hover:bg-[var(--pop)] hover:text-[var(--teritary)] p-2 text-[var(--primary)]'>
              <FontAwesomeIcon icon={contact.icon} />
              {contact.name}
            </Link>
          )})}
          <Link target="_blank" href='/YenLamResume2024v3.pdf' download={"YenLamResume2024v3.pdf"} className='box flex items-center gap-x-2 rounded-lg hover:translate-y-[2px] hover:bg-[var(--pop)] hover:text-[var(--teritary)] p-2 text-[var(--primary)]'>
        ...
        <ul className='text-2xl text-[var(--primary)] flex gap-4 items-center justify-evenly'>
          {contacts.map((contact, index)=>{ return (
            <Link key={index} href={contact.url} className='flex group hover:translate-y-[2px] p-2'>
              <FontAwesomeIcon icon={contact.icon} />
            </Link>
          )})}
