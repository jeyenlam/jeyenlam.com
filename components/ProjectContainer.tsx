...
const ProjectContainer: React.FC<ProjectContainerProps> = ({project}) => {
  return (
    <div className='mt-2 rounded-2xl bg-zinc-50'>
      {/* Title */}
      <h1 className='bg-lime-200 pl-2 p-1 sm:p-2 text-zinc-800 text-sm sm:text-base font-medium sm:font-semibold flex justify-start rounded-t-2xl zinc-900'>{project.title}</h1>
      {/* Content */}
      <div className='p-2 md:p-6 text-xs flex flex-col gap-3'>
        ...
        <ul className='flex text-xl sm:text-2xl gap-2 text-zinc-800'>
          {(project.urls).map((url, index) => {return(
            <li key={index} className='p-[2px] px-[6px] rounded-xl hover:text-3xl zinc-900'><Link href={url.url} target="_blank"><FontAwesomeIcon icon={url.icon}/></Link></li>
          )})}
        </ul>
